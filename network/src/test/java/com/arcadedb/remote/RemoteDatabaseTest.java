...
class RemoteDatabaseTest {
    ...
    void testDatabases() throws Exception {
        ...
        database.databases();
        verify(database).httpCommand(eq("POST"), eq(null), eq("server"), eq(null),
                eq("list databases"), eq(null), eq(true), eq(true), any());
        verify(database).createConnection("POST", "http://localhost:1234/api/v1/server");
    ...
    void testTransactionNotJoined() throws Exception {
        ...
        boolean createdNewTx = database.transaction(()-> {}, false, 1);
    ...
    void testTransactionJoined() throws Exception {
        ...
        boolean createdNewTx = database.transaction(()-> {}, true, 1);
    ...
    void testTransactionRollback() throws Exception {
        ...
        assertThrows(RuntimeException.class, () -> database.transaction(()-> {}, false, 1));
        verify(database).begin();
    ...
    void testCommand() throws Exception {
        ...
        verify(database).httpCommand(eq("POST"), eq("testdb"), eq("command"), eq("SQL"),
                eq(command), eq(null), eq(true), eq(true), any());
    ...
    void testCommandWithParameters() throws Exception {
        ...
        verify(database).httpCommand(eq("POST"), eq("testdb"), eq("command"), eq("SQL"),
                eq(command), eq(paramsMap), eq(true), eq(true), any());
        verify(database).createConnection("POST", "http://localhost:1234/api/v1/command/testdb");
        JSONObject payload = new JSONObject(
                "{\"language\":\"SQL\",\"command\":\""
                        + command + "\",\"serializer\":\"record\",\"params\":"
