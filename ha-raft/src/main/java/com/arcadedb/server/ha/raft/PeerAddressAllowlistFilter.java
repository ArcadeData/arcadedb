 ...
 * can inject log entries" attack out of the box, without requiring certificate provisioning.
 * <p>
...
final class PeerAddressAllowlistFilter extends ServerTransportFilter {
  ...
  static List<String> extractPeerHosts(final String serverList) {
    ...
    for (final String entry : entries) {
      ...
      if (trimmed.isEmpty())
        continue;
      final String[] parts = RaftPeerAddressResolver.parseHostPort(trimmed);
      String host = parts[0];
      // Strip brackets from IPv6 hosts for InetAddress.getAllByName compatibility.
