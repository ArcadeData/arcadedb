...
/**
 * Fault-injection test for the narrow crash window documented in
 * {@link ArcadeDBStateMachine#applyTransactionEntry(byte[])}: the leader crashes AFTER
 * Ratis has committed the log entry (so followers apply it via the state machine) but
 * BEFORE {@code commit2ndPhase()} runs locally on the leader.
 *
 * <p>Execution order on the leader ({@link ReplicatedDatabase#commit()}):
 * <ol>
 *   <li>{@code commit1stPhase()} - WAL prepared</li>
 *   <li>{@code replicateTransaction()} blocks waiting for Raft quorum</li>
 *   <li>Raft gets MAJORITY ack and commits the entry (durable on majority)</li>
 *   <li>{@code applyTransaction()} fires on the leader state machine (origin-skip)</li>
 *   <li>Ratis returns the client reply</li>
 *   <li>{@code replicateTransaction()} returns to the caller &lt;-- crash injected HERE</li>
 *   <li>{@code commit2ndPhase()} would write pages locally (skipped by fault injection)</li>
 * </ol>
 *
 * <p>On restart the old leader must recover its missed page writes via the Raft state
 * machine replay path. At replay time {@code isLeader()} returns false, so the
 * origin-skip in {@code applyTransactionEntry()} does NOT fire and the follower apply
 * path runs. The page-version guard makes the apply idempotent.
 *
 * <p>Assertions:
 * <ol>
 *   <li>The injected entry is visible on the new leader (proves majority applied it)</li>
 *   <li>The restarted old leader has the same record count (proves replay recovered it)</li>
 *   <li>{@code DatabaseComparator} finds all 3 nodes identical (no double-apply corruption)</li>
 * </ol>
 *
...
@Tag("IntegrationTest")
@Tag("slow")
class RaftLeaderCrashBetweenCommitAndApplyIT extends BaseGraphServerTest {
  ...
  void clearPostReplicationHook() {
    ReplicatedDatabase.TEST_POST_REPLICATION_HOOK = null;
  }
  ...
  void oldLeaderRecoversMissingWriteAfterCrashBetweenReplicationAndPhase2() throws Exception {
    ...
    //      same thread that is mid-commit would deadlock / corrupt the Ratis gRPC channel
    ...
    ReplicatedDatabase.TEST_POST_REPLICATION_HOOK = dbName -> {
    ...
    // with no retry loop so the fault injection fires exactly once.
    try {
      leaderDb.begin();
      final MutableVertex v = leaderDb.newVertex(VERTEX_TYPE);
    ...
    // the precondition for the crash window we are exercising.
    ...
    // but commit2ndPhase() never ran so the pages are missing. Ratis replay applies the
    // entry via the state machine follower path (origin-skip bypassed because isLeader
    // is false at replay time).
    Thread.sleep(2_000); // release gRPC port
    ...
    // Phase 8: full cross-node convergence check. DatabaseComparator walks records,
    // indexes and buckets and would fail on any double-apply corruption or divergence.
    waitForReplicationConvergence();
