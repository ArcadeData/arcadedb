...
class LocalCommitRegistryTest {
  ...
  void theApplyThreadClaimsARegisteredTransactionExactlyOnce() {
    final LocalCommit commit = registry.register(new LocalCommit(DB, 7L, mock(TransactionContext.class), null));
    assertThat(registry.size()).isEqualTo(1);

    assertThat(registry.claim(DB, 7L)).isSameAs(commit);
    assertThat(commit.isClaimed()).isTrue();
    assertThat(registry.size()).as("a claim consumes the registration").isZero();

    assertThat(registry.claim(DB, 7L)).as("a replay of the same entry finds nothing to claim").isNull();
  ...
  void theCommittingThreadWithdrawsBeforeTheClaim() {
    ...
    assertThat(registry.claim(DB, 7L)).as("a withdrawn transaction is applied from its WAL bytes").isNull();
  ...
  void registrationsAreKeyedByDatabaseAndTransactionId() {
    registry.register(new LocalCommit(DB, 7L, mock(TransactionContext.class), null));
    assertThat(registry.claim("other", 7L)).isNull();
    assertThat(registry.claim(DB, 8L)).isNull();
    assertThat(registry.size()).isEqualTo(1);
  ...
  void theOutcomeIsAwaitedByTheCommittingThread() throws InterruptedException {
    ...
    final LocalCommit failed = registry.register(new LocalCommit(DB, 8L, mock(TransactionContext.class), null));
  ...
  void onlyOneSideWinsUnderContention() throws InterruptedException {
    ...
    for (int round = 0; round < rounds; round++) {
      final LocalCommit commit = registry.register(new LocalCommit(DB, round, mock(TransactionContext.class), null));
      final CountDownLatch start = new CountDownLatch(1);
      final long walTxId = round;
      final List<Thread> threads = new ArrayList<>();
      threads.add(Thread.ofPlatform().unstarted(() -> {
        await(start);
        if (registry.claim(DB, walTxId) != null)
