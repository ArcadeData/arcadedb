...
class BootstrapElectionSourceSelectionTest {
  ...
  void identicalCopiesPreferTheLocalPeer() {
    ...
    final Map<String, List<PeerState>> states = new HashMap<>();
    states.put("db1", List.of(state(A, "db1", 10), state(B, "db1", 10), state(C, "db1", 10)));
    states.put("db2", List.of(state(A, "db2", 10), state(B, "db2", 10), state(C, "db2", 10)));
    states.put("db3", List.of(state(A, "db3", 10), state(B, "db3", 10), state(C, "db3", 10)));
  ...
  void freshestNodeWinsAcrossAllDatabases() {
    ...
    final Map<String, List<PeerState>> states = new HashMap<>();
    states.put("db1", List.of(state(A, "db1", 20), state(B, "db1", 18), state(C, "db1", 15)));
    states.put("db2", List.of(state(A, "db2", 30), state(B, "db2", 30), state(C, "db2", 25)));
    states.put("db3", List.of(state(A, "db3", 12), state(B, "db3", 9), state(C, "db3", 12)));
  ...
  void pluralityElectsASingleSourceInsteadOfAbandoningDatabases() {
    ...
    final Map<String, List<PeerState>> states = new HashMap<>();
    states.put("db1", List.of(state(A, "db1", 50), state(B, "db1", 40)));
    states.put("db2", List.of(state(A, "db2", 50), state(B, "db2", 40)));
    states.put("db3", List.of(state(A, "db3", 40), state(B, "db3", 50)));
  ...
  void aggregateBeatsPreferSelfOnVoteTie() {
    ...
    final Map<String, List<PeerState>> states = new HashMap<>();
    states.put("db1", List.of(state(A, "db1", 10), state(B, "db1", 10))); // tie -> A,B vote
    states.put("db2", List.of(state(A, "db2", 10), state(B, "db2", 10))); // tie -> A,B vote
    // db3: C is freshest (both A and B only contribute to aggregate, not votes), and A > B here.
    states.put("db3", List.of(state(A, "db3", 8), state(B, "db3", 4), state(C, "db3", 20)));
  ...
  void lexicographicIdBreaksFullTies() {
    ...
    final Map<String, List<PeerState>> states = new HashMap<>();
    states.put("db1", List.of(state(A, "db1", 7), state(B, "db1", 7)));
    states.put("db2", List.of(state(A, "db2", 7), state(B, "db2", 7)));
  ...
  void noDataAnywhereReturnsNull() {
    ...
    final Map<String, List<PeerState>> states = new HashMap<>();
    states.put("db1", List.of(state(A, "db1", -1), state(B, "db1", -1)));
    states.put("db2", List.of(state(A, "db2", -1), state(B, "db2", -1)));
  ...
  void databasesWithoutDataDoNotVote() {
    ...
    final Map<String, List<PeerState>> states = new HashMap<>();
    states.put("db1", List.of(state(A, "db1", -1), state(B, "db1", 5)));
    states.put("db2", List.of(state(A, "db2", -1), state(B, "db2", -1)));
