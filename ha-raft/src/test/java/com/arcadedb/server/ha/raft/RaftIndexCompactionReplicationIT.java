...
/**
 * Documents known limitations of index compaction in the Raft HA implementation.
 * All tests are disabled because index compaction is not replicated via Raft log entries
 * ({@link RaftLogEntryType} has no {@code COMPACT} entry type).
 * <p>
 * These tests serve as a regression specification: once compaction replication is
 * implemented, they should be enabled and verified.
 *
...
class RaftIndexCompactionReplicationIT extends BaseRaftHATest {
   ...
   * (no COMPACT log entry type in {@link RaftLogEntryType}), so follower consistency
