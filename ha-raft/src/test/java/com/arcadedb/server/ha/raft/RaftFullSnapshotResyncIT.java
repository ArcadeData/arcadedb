...
/**
 * Integration test: 3-node cluster with a low snapshot threshold.
 ...
 * <p>
 * <strong>Status: disabled pending {@code ArcadeDBStateMachine.installSnapshot()} implementation.</strong>
 * <p>
 * {@link ArcadeDBStateMachine#takeSnapshot()} is implemented and returns the last-applied
 * index so Ratis can compact the log (reducing disk usage). However, snapshot-based
 * resync requires the snapshot install path to transfer actual ArcadeDB database files
 * from the leader to the lagging replica. Until that is implemented, a replica whose
 * Raft log entries have been purged post-snapshot cannot catch up and will fail to rejoin.
 * <p>
 ...
 */
@Disabled("Snapshot-based resync requires ArcadeDBStateMachine.installSnapshot() - not yet implemented")
