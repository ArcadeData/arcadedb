...
class PostVerifyDatabaseHandlerTest {
  ...
  private static final Pattern VALID_DATABASE_NAME;

  static {
    try {
      final Field f = PostVerifyDatabaseHandler.class.getDeclaredField("VALID_DATABASE_NAME");
      f.setAccessible(true);
      VALID_DATABASE_NAME = (Pattern) f.get(null);
  ...
  void validDatabaseNamesMatch(final String name) {
    assertThat(VALID_DATABASE_NAME.matcher(name).matches()).isTrue();
  }
  ...
  void invalidDatabaseNamesAreRejected(final String name) {
    assertThat(VALID_DATABASE_NAME.matcher(name).matches()).isFalse();
  }

  @Test
  void pathTraversalSequencesAreRejected() {
    assertThat(VALID_DATABASE_NAME.matcher("..").matches()).isFalse();
    assertThat(VALID_DATABASE_NAME.matcher("../..").matches()).isFalse();
    assertThat(VALID_DATABASE_NAME.matcher("foo/../bar").matches()).isFalse();
  }
}
