  ...
  public void testDuplicateKeyIndexWithOrderBy() {
    database.transaction(() -> {
      ...
      for (int i = 0; i < 75; i++) {
        ...
        // Let's also check what's in the index after each save
        if (i < 5) {
          try (ResultSet rs = database.query("sql", "SELECT count(*) FROM metadata")) {
            Result result = rs.next();
            long count = (Long) result.getProperty("count(*)");
            System.out.println("After inserting document " + i + ", total count: " + count);
          }
        }
      }
    });
    ...
    database.transaction(() -> {
      ...
      // Test 4: This is the problematic case - indexed column in both projection and ORDER BY
      try (ResultSet rs = database.query("sql", "EXPLAIN SELECT name, publicationYear FROM metadata ORDER BY publicationYear")) {
        while (rs.hasNext()) {
          Result result = rs.next();
          System.out.println("Execution plan: " + result);
        }
      }
      ...
      try (ResultSet rs = database.query("sql", "SELECT name, publicationYear FROM metadata ORDER BY publicationYear")) {
        ...
        System.out.println("Duplicated names: " + nameCount.entrySet().stream().filter(e -> e.getValue() > 1).count());
        // This assertion should fail with the current bug, showing we get more than 75 records
        assertThat(count).withFailMessage("Expected 75 records but got " + count + " - this indicates the duplicate index bug").isEqualTo(75);
      }
    });
  }
}