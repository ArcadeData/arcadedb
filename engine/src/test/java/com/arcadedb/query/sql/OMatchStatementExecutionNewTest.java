...
public class OMatchStatementExecutionNewTest extends TestHelper {
    public OMatchStatementExecutionNewTest() {
        autoStartTx = true;
    }
...
//    database.query("sql", "select expand(out()) from IndexedVertex where uid = 0").stream().forEach(x -> System.out.println("x = " + x));
    ...
    public void testOrderByOutOfProjDesc() {
        String type = "testOrderByOutOfProjDesc";

        database.execute("sql",
                "BEGIN;" +
                        "CREATE vertex type " + type + ";" +
                        "CREATE VERTEX " + type + " SET name = 'aaa', num = 0, num2 = 1;" +
                        "CREATE VERTEX " + type + " SET name = 'aaa', num = 1, num2 = 2;" +
                        "CREATE VERTEX " + type + " SET name = 'aaa', num = 2, num2 = 3;" +
                        "COMMIT;  ");

        String query = "MATCH { type: " + type + ", as:a} RETURN a.name as name, a.num as num order by a.num2 desc";

        ResultSet resultSet = database.query("sql", query);
        List<Result> results = resultSet
                .stream().collect(Collectors.toList());

        assertThat(results).hasSize(3);

        assertThat(results.get(0).<String>getProperty("name")).isEqualTo("aaa");
        assertThat(results.get(0).<Integer>getProperty("num")).isEqualTo(2);
        assertThat(results.get(1).<String>getProperty("name")).isEqualTo("aaa");
        assertThat(results.get(1).<Integer>getProperty("num")).isEqualTo(1);
        assertThat(results.get(2).<String>getProperty("name")).isEqualTo("aaa");
        assertThat(results.get(2).<Integer>getProperty("num")).isEqualTo(0);
