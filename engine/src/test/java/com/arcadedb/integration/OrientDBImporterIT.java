    ...
    try {
      ...
      try (final DatabaseFactory factory = new DatabaseFactory(DATABASE_PATH)) {
        try (Database database = factory.open()) {
          ...
          Assertions.assertEquals(SchemaImpl.INDEX_TYPE.LSM_TREE, database.getSchema().getIndexByName("Person[id]").getType());
          ...
          Assertions.assertEquals(SchemaImpl.INDEX_TYPE.LSM_TREE, database.getSchema().getIndexByName("Friend[id]").getType());
