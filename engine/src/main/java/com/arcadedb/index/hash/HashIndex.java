    ...
    if (status.compareAndSet(INDEX_STATUS.AVAILABLE, INDEX_STATUS.UNAVAILABLE)) {
      ...
      db.scanBucket(db.getSchema().getBucketById(metadata.associatedBucketId).getName(), record -> {
        ...
        if (exception instanceof RuntimeException re)
          throw re;
        throw new IndexException("Error on building hash index '" + name + "' at record " + rid, exception);
      });
  ...
  private Object[] convertKeys(final Object[] keys) {
    if (keys != null) {
      ...
      for (int i = 0; i < keys.length; ++i) {
        if (keys[i] == null)
          continue;
        // convertOrNull(), not convert(): the twin of LSMTreeIndexAbstract#convertKeysToDeclaredTypes and for the
        // same reason. The keys reaching here are whatever the records hold, and on a schemaless property that can
        // include a value of an entirely different shape than the type the index settled on. Such a row indexes
        // under a null key and the build carries on; refusing it would make one heterogeneous record fail
        // CREATE INDEX outright - and, since build() rethrows, would also fail an ordinary INSERT that used to
        // index a null key and continue (issue #8090, which made convert() itself strict).
        convertedKeys[i] = BinaryComparator.canonicalizeForByteEquality(
            Type.convertOrNull(getDatabase(), keys[i], BinaryTypes.getClassFromType(keyTypes[i])));
