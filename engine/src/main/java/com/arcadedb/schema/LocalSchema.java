...
public class LocalSchema implements Schema {
  ...
  protected <RET> RET recordFileChanges(final Callable<Object> callback) {
    ...
    final long generationBefore = dirtyGeneration.get();
    ...
    try {
      ...
      if (!executed)
        // ROLLBACK: restore the generation to before this change. Uses CAS to avoid
        // clobbering a concurrent increment from another thread.
        dirtyGeneration.compareAndSet(generationBefore + 1, generationBefore);
    }
  }
