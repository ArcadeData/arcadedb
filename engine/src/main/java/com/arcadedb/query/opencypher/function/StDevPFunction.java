...
/**
 * stDevP() aggregation function - computes the population standard deviation.
 * Divides by n (no Bessel's correction).
 * Returns 0.0 for a single value, null for no values.
 * Example: MATCH (n:Val) RETURN stDevP(n.v)
 *
 * @author Luca Garulli (l.garulli--(at)--arcadedata.com)
 */
public class StDevPFunction implements StatelessFunction {
  private long   n;
  private double mean;
  private double m2;
  ...
  public Object execute(final Object[] args, final CommandContext context) {
    if (args.length != 1)
      throw new CommandExecutionException("stDevP() requires exactly one argument");
