...
public final class CypherExpressionWalker {
  ...
  public static void walk(final Expression expr, final Visitor visitor) {
    ...
    if (expr instanceof FunctionCallExpression func) {
    ...
    } else if (expr instanceof CypherExpressionBuilder.ChainedPropertyAccessExpression chained) {
      walk(chained.getBaseExpression(), visitor);
      ...
      walkNested(exists.getParsedSubquery(), visitor);
    } else if (expr instanceof CountExpression count) {
      walkNested(count.getParsedSubquery(), visitor);
    } else if (expr instanceof CollectExpression collect) {
      walkNested(collect.getParsedSubquery(), visitor);
    }
    // else: a leaf - literal, variable, parameter or property access.
  }
  ...
  public static void walk(final BooleanExpression expr, final Visitor visitor) {
    ...
    // else: a predicate with no nested expression to reach.
  }
