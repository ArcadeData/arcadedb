...
public class DatabaseContext extends ThreadLocal<Map<String, DatabaseContext.DatabaseContextTL>> {
  ...
  // KEYED BY Thread.getId(): ON THE JDK 21+ MAIN-LINE BASELINE THIS CLASS IS KEYED BY Thread.threadId(), WHICH
  // IS FORMALLY SPECIFIED AS A POSITIVE ID GENERATED WHEN THE THREAD IS CREATED AND NEVER REUSED. threadId()
  // DOES NOT EXIST ON THIS JAVA 17 BRANCH (ADDED IN JDK 19), SO Thread.getId() IS USED INSTEAD - IN THE HOTSPOT
  // IMPLEMENTATION ON JDK 17 IT IS BACKED BY THE SAME MONOTONIC, NEVER-REUSED PER-THREAD COUNTER AS threadId()
  // ON LATER JDKS (getId()'s OLDER JAVADOC PERMITTED REUSE BUT THE ACTUAL IMPLEMENTATION DOES NOT DO SO). EVERY
  // EXACTLY-ONCE CLAIM/REMOVE ARGUMENT IN THIS CLASS (THE SWEEP'S KEYED CLAIM, removeContext,
  // removeCurrentThreadContexts) DEPENDS ON THIS NON-REUSE: DO NOT REFACTOR TO ANY OTHER REUSABLE KEY, AND
  // RE-VERIFY THE GUARANTEE ON JDK MAJOR BUMPS (INCLUDING A FUTURE MERGE FORWARD TO threadId() ON JDK 21+)
  ...
  public DatabaseContextTL init(final DatabaseInternal database, final TransactionContext firstTransaction) {
    ...
    final ThreadContexts registered = CONTEXTS.get(currentThread.getId());
    if (registered == null || registered.contexts != map)
      CONTEXTS.put(currentThread.getId(), new ThreadContexts(currentThread, map));
  ...
  public void removeCurrentThreadContexts() {
    ...
    // THREAD IDS NEVER REUSED (SEE THE getId() PROVENANCE NOTE ON CONTEXTS), THE SWEEP ONLY CLAIMS
    // DEAD THREADS
    CONTEXTS.remove(Thread.currentThread().getId());
  }
