...
**Once a record outgrows its page, none of that applies to it any more.** A `FIRST_CHUNK` slot, a placeholder pointer that has to be rebuilt, and a placeholder CONTENT record growing on its own page are all outside the merge, so each of them poisons its page unconditionally. On a single-bucket type whose records have grown past the page size that means concurrent updates of *unrelated* records conflict for good - the very false conflict #5279 removed, one size class up - and N-way contention degenerates into one winner per round. The `TX_RETRIES` default of 3 is sized for incidental contention and is not enough there: `Issue5279ConcurrentUpdateTest.growingUpdatesUnderContentionKeepTheDatabaseConsistent` needs an explicit budget for exactly this reason, and it is a contention regime, not corruption (#6129).
