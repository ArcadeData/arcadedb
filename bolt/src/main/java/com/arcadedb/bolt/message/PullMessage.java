...
/**
 * PULL message to fetch records from result stream.
 * Signature: 0x3F
 * Fields: extra (Map containing n, qid)
 */
public class PullMessage extends BoltMessage {
  private final Map<String, Object> extra;

  public PullMessage(final Map<String, Object> extra) {
    super(PULL);
  ...
  /**
   * Get the number of records to pull. -1 means all records.
   */
  public long getN() {
    final Object n = extra.get("n");
    if (n == null) {
      return -1; // Default: pull all
    }
    return ((Number) n).longValue();
  }

  /**
   * Get the query ID (qid) for which to pull records.
  ...
  public String toString() {
    return "PULL{n=" + getN() + ", qid=" + getQid() + ", extra=" + extra + "}";
  }
}
