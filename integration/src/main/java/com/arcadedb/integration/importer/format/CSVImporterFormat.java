...
public class CSVImporterFormat extends AbstractImporterFormat {
  ...
  private void loadDocuments(final SourceSchema sourceSchema, final Parser parser, final Database database,
    ...
    // CAME BEFORE IT, AT THE COST OF THE MACRO-BATCHING abort MODE STILL GETS. THE ONLY THING THAT CAN THROW BETWEEN A
    // ROW'S database.begin() AND ITS OWN catch/commit IS THIS METHOD'S RuntimeException-CATCHING ROW LOGIC ITSELF -
    // csvParser.parseNext() (THE OTHER THROWING CALL IN THIS METHOD) ONLY RUNS AT THE TOP OF THE NEXT LOOP ITERATION,
    // I.E. RIGHT AFTER THE PRIOR ROW'S TRANSACTION WAS ALREADY COMMITTED/ROLLED BACK AND A FRESH, EMPTY ONE BEGUN - SO
    // A csvParser.parseNext() IOException CAN NEVER LEAVE A ROW'S PARTIAL WRITE SITTING IN AN OPEN TRANSACTION.
