...
public final class ArcadeTraversalBinder {
  ...
  static final class IllegalTraversalBindingsEntry extends IllegalArgumentException {
    private IllegalTraversalBindingsEntry(final String message) {
      super(message);
    }
  }
  ...
  public interface TraversalSupplier {
     ...
     */
    static <T extends TraversalSource> TraversalSupplier of(Class<T> klass) {
   ...
   */
  ArcadeTraversalBinder(final ArcadeGraph arcadeGraph) {
    graph = arcadeGraph;
    traversalBindings = arcadeGraph.getDatabase().getConfiguration().getValue(GlobalConfiguration.GREMLIN_TRAVERSAL_BINDINGS);
    validateTraversalBindings(traversalBindings);
  }

  void bind(final GremlinScriptEngine engine) {
    traversalBindings.entrySet().forEach(e -> {
      final var bindingName = e.getKey();
      final var supplier = e.getValue();
      engine.put(bindingName, supplier.get(graph));
    });
  }

  private static void validateTraversalBindings(final Map<String, TraversalSupplier> bindings) {
    bindings.entrySet().forEach(entry -> {
      try {
        ...
        throw new IllegalTraversalBindingsEntry(
            String.format("illegal Map<String, TraversalSupplier> entry in GREMLIN_TRAVERSAL_BINDINGS configuration: %s", entry));
      }
    });
  }
}
