...
/**
 * This implementation extends the default one in Gremlin to avoid throwing exception in case the comparison is between different types or just with a null value.
 * Throwing exceptions in Java is expensive and should be avoided for such cases.
 *
...
public abstract class GremlinValueComparator implements Comparator<Object> {
  ...
  public static final GremlinValueComparator ORDERABILITY = new GremlinValueComparator() {
    ...
    public int compare(final Object f, final Object s) {
       ...
       */
      final Type st = f.getClass().equals(s.getClass()) || (f instanceof Number && s instanceof Number) ? ft : Type.type(s);
  ...
  public static final GremlinValueComparator COMPARABILITY = new GremlinValueComparator() {
    ...
    private boolean containersOfDifferentSize(final Object f, final Object s) {
      if (f instanceof Collection collection && s instanceof Collection collection)
        if (collection.size() != (collection.size()))
          return true;

      if (f instanceof Path path && s instanceof Path path)
   ...
   */
  private final Comparator<Property> propertyComparator = Comparator.<Property, Object>comparing(Property::key, this).thenComparing(Property::value, this);
   ...
   */
  private final Comparator<Map.Entry> entryComparator = Comparator.<Map.Entry, Object>comparing(Map.Entry::getKey, this)
      .thenComparing(Map.Entry::getValue, this);
   ...
   */
  public enum Type {
    Nulltype, Boolean(Boolean.class), Number(Number.class), Date(Date.class), String(String.class), UUID(UUID.class), Vertex(Vertex.class), Edge(
        Edge.class), VertexProperty(VertexProperty.class), Property(Property.class), Path(Path.class), Set(Set.class), List(List.class), Map(
        Map.class), MapEntry(Map.Entry.class), Unknown(Object.class);
