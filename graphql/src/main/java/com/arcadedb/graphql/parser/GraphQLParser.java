...
public class GraphQLParser/*@bgen(jjtree)*/implements GraphQLParserTreeConstants, GraphQLParserConstants {/*@bgen(jjtree)*/
  protected JJTGraphQLParserState jjtree = new JJTGraphQLParserState();private int tokenId = 0;
    ...
    /**
     * Bounds how deeply '{' ... '}' can nest in the document - selection sets and object/interface/input
     * type bodies both use the same delimiter - before the recursive-descent parser runs. Every level of
     * nesting re-enters the SelectionSet/Field grammar rules (or the equivalent type-definition ones) at
     * least once, so a document with enough nesting exhausts the calling thread's native stack with a
     * StackOverflowError instead of a normal parse error: 3085 levels does it with an ~18 KB payload on a
     * default JVM stack. Counting brace depth on the token stream is a precise, allocation-cheap proxy for
     * that recursion and runs before any parse tree is built, using a dedicated token manager so the real
     * parse below still starts from a fresh, unconsumed char stream. GraphQL string literals and '#'
     * comments are each lexed as a single token, so a brace inside either one is never counted here. See
     * issue #5853.
     */
        ...
        if (token.kind == OCBR) {
          if (++depth > maxDepth)
            throw new ParseException("GraphQL document nesting exceeds the maximum allowed depth of " + maxDepth
                + " ('{' nested inside one another). This protects the server against a stack overflow from a "
                + "pathologically nested query; raise 'arcadedb.graphql.maxNestingDepth' if this is a legitimate query.");
        } else if (token.kind == CCBR && depth > 0) {
          --depth;
        }
      }
    }
