...
/**
 * Base for the single-server tests that start the gRPC plugin. The plugin is started on port {@code 0}, so the
 * operating system assigns a free port, and every test must connect to {@link #getServerGrpcPort()} rather than to
 * 50051 or to the configured {@link GlobalConfiguration#GRPC_PORT}: anything already listening on the production
 * default (a developer's own ArcadeDB, a second concurrent build, another agent) used to fail the module at
 * {@code @BeforeEach} or, worse, answer the test's connections (issue #8209).
 *
 * @author Luca Garulli (l.garulli@arcadedata.com)
 */
public abstract class BaseGrpcServerTest extends BaseGraphServerTest {
