...
$jscomp.polyfill(
  ...
  function (b) {
    ...
    h.prototype.set = function (a, d) {
      ...
      c.entry
        ? (c.entry.value = d)
        : ((c.entry = { next: this.head_, previous: this.head_.previous, head: this.head_, key: a, value: d }),
          c.list.push(c.entry),
...
(function () {
    ...
    m = (function () {
      function f(a, d, g, c, e, r) {
        ...
        this.s = {
          ...
          lastSelect: !1,
          redraw: !1,
          rowData: { arrayFilter: [], arrayOriginal: [], arrayTotals: [], bins: {}, binsOriginal: {}, binsTotal: {}, filterMap: new Map() },
        ...
        this.dom = {
          ...
          container: $("<div/>")
            ...
            .addClass(this.classes.layout + (7 > d ? c : c.split("-")[0] + "-6")),
          countButton: $('<button type="button"></button>').addClass(this.classes.paneButton).addClass(this.classes.countButton),
          dtP: $(
            "<table><thead><tr><th>" +
              (this.colExists ? $(a.column(this.colExists ? this.s.index : 0).header()).text() : this.customPaneSettings.header || "Custom Pane") +
        ...
        this.s.searchFunction = function (a, c, d, e) {
          ...
          n.colExists &&
            ((a = c[n.s.index]),
            "filter" !== l.orthogonal.filter && ((a = n.s.rowData.filterMap.get(d)), a instanceof $.fn.dataTable.Api && (a = a.toArray())));
      ...
      f.prototype.clearData = function () {
        this.s.rowData = { arrayFilter: [], arrayOriginal: [], arrayTotals: [], bins: {}, binsOriginal: {}, binsTotal: {}, filterMap: new Map() };
      };
      ...
      f.prototype.destroy = function () {
        ...
        for (var a = $.fn.dataTable.ext.search.indexOf(this.s.searchFunction); -1 !== a; )
          $.fn.dataTable.ext.search.splice(a, 1), (a = $.fn.dataTable.ext.search.indexOf(this.s.searchFunction));
      ...
      f.prototype._addOption = function (a, d, g, c, e, r) {
        if (Array.isArray(a) || a instanceof b.Api)
          if ((a instanceof b.Api && ((a = a.toArray()), (d = d.toArray())), a.length === d.length))
            for (var f = 0; f < a.length; f++) r[a[f]] ? r[a[f]]++ : ((r[a[f]] = 1), e.push({ display: d[f], filter: a[f], sort: g, type: c }));
          else throw Error("display and filter not the same length");
        else
          "string" === typeof this.s.colOpts.orthogonal
            ? r[a]
              ? r[a]++
              : ((r[a] = 1), e.push({ display: d, filter: a, sort: g, type: c }))
      ...
      f.prototype._adjustTopRow = function () {
        ...
        (252 > $(a[0]).width() || 252 > $(c[0]).width()) && 0 !== $(a[0]).width()
          ? ($(a[0]).addClass(this.classes.narrow),
            $(d[0]).addClass(this.classes.narrowSub).removeClass(this.classes.narrowSearch),
            $(g[0]).addClass(this.classes.narrowSub).removeClass(this.classes.narrowButton))
          : ($(a[0]).removeClass(this.classes.narrow),
            $(d[0]).removeClass(this.classes.narrowSub).addClass(this.classes.narrowSearch),
            $(g[0]).removeClass(this.classes.narrowSub).addClass(this.classes.narrowButton));
      };
      f.prototype._buildPane = function () {
        ...
        if (this.colExists) {
          ...
          if ((!1 === c.show || (void 0 !== c.show && !0 !== c.show)) && -1 === q)
              ...
              columnDefs: [
                {
                  ...
                  render: function (d, e, g) {
                    ...
                    var b;
                    a.s.filteringActive && a.c.viewTotal ? (b = f.replace(/{total}/, g.total)) : (b = r.replace(/{total}/, g.total));
                    ...
                    return a.c.dataLength
                      ? d.length > a.c.dataLength
                        ? '<span class="' + a.classes.name + '">' + d.substr(0, a.c.dataLength) + "...</span>" + e
                        : '<span class="' + a.classes.name + '">' + d + "</span>" + e
                      : '<span class="' + a.classes.name + '">' + d + "</span>" + e;
                  },
                  targets: 0,
                  type: void 0 !== d.settings()[0].aoColumns[this.s.index] ? d.settings()[0].aoColumns[this.s.index]._sManualType : null,
                },
        ...
        if (this.colExists) {
          ...
          for (g = e.arrayFilter.length; p < g; p++)
            !e.arrayFilter[p] || (void 0 === e.bins[e.arrayFilter[p].filter] && this.c.cascadePanes)
              ? this._addRow(this.c.emptyMessage, k, k, this.c.emptyMessage, this.c.emptyMessage, this.c.emptyMessage)
              : ((q = this._addRow(
                ...
                void 0 !== c.preSelect && -1 !== c.preSelect.indexOf(e.arrayFilter[p].filter) && q.select());
        }
        ...
        this.s.dt.on("stateSaveParams.dtsp", function (c, d, g) {
          if ($.isEmptyObject(g)) a.s.dtPane.state.clear();
            ...
            g.searchPanes.panes.push({ arrayFilter: n, bins: r, id: a.s.index, order: f, searchTerm: b, selected: c });
          }
        });
        if (l && l.searchPanes && l.searchPanes.panes)
          for (this.c.cascadePanes || this._reloadSelect(l), g = 0, l = l.searchPanes.panes; g < l.length; g++)
            (p = l[g]), p.id === this.s.index && ($(this.dom.searchBox).val(p.searchTerm), this.s.dt.order(p.order));
      ...
      f.prototype._detailsPane = function () {
        ...
        d.rows().every(function (c) {
          a._populatePaneArray(c, a.s.rowData.arrayTotals, g, a.s.rowData.binsTotal);
        });
      };
      ...
      f.prototype._findUnique = function (a, d) {
        for (var g = [], c = 0; c < d.length; c++) {
          var e = d[c];
          -1 === g.indexOf(e.filter) && (a.push({ display: e.display, filter: e.filter, sort: e.sort, type: e.type }), g.push(e.filter));
        }
      };
      f.prototype._getBonusOptions = function () {
        return $.extend(!0, {}, f.defaults, { orthogonal: { threshold: null }, threshold: null }, void 0 !== this.c ? this.c : {});
      };
      ...
      f.prototype._getOptions = function () {
        return $.extend(!0, {}, f.defaults, { orthogonal: { threshold: null }, threshold: null }, this.s.dt.settings()[0].aoColumns[this.s.index].searchPanes);
      };
      ...
      f.prototype._populatePane = function () {
        ...
        var d = this.s.dt.settings()[0],
          g = 0;
        for (
          a = (!this.c.cascadePanes && !this.c.viewTotal) || this.s.clearing ? a.rows().indexes() : a.rows({ search: "applied" }).indexes();
          g < a.length;
          g++
        )
      ...
      f.prototype._populatePaneArray = function (a, d, g, c) {
        ...
        if ("string" === typeof e.orthogonal)
          (g = g.oApi._fnGetCellData(g, a, this.s.index, e.orthogonal)), this.s.rowData.filterMap.set(a, g), this._addOption(g, g, g, g, d, c);
        else {
              ...
              var e = function (a) {
                  if (a && ((void 0 !== c.bins[a.filter] && 0 !== c.bins[a.filter] && b.c.cascadePanes) || !b.c.cascadePanes || b.s.clearing)) {
                    var d = b._addRow(
                        a.display,
                        a.filter,
                        b.c.viewTotal ? (void 0 !== c.bins[a.filter] ? c.bins[a.filter] : 0) : c.bins[a.filter],
                        b.c.viewTotal ? String(c.binsTotal[a.filter]) : c.bins[a.filter],
                        a.sort,
                        a.type,
                      ),
    ...
    h = (function () {
      ...
      b.prototype.redrawPanes = function () {
        ...
        if (!this.s.updating) {
          var d = !0,
            b = this.s.filterPane;
          if (a.rows({ search: "applied" }).data().toArray().length === a.rows().data().toArray().length) d = !1;
          ...
          if (this.regenerating) {
            ...
            if (0 < this.s.selectionList.length)
              for (a = this.s.selectionList[this.s.selectionList.length - 1].index, n = 0, l = this.s.panes; n < l.length; n++)
                (f = l[n]), (f.s.lastSelect = f.s.index === a && 1 === this.s.selectionList.length);
            for (f = 0; f < this.s.selectionList.length; f++)
              if (this.s.selectionList[f].index !== e || !0 === this.s.selectionList[f].protect) {
                a = !1;
                for (n = f + 1; n < this.s.selectionList.length; n++) this.s.selectionList[n].index === this.s.selectionList[f].index && (a = !0);
            ...
            if (0 < c.length && c.length < this.s.selectionList.length)
              for (this._cascadeRegen(c), a = c[c.length - 1].index, b = 0, c = this.s.panes; b < c.length; b++)
                (f = c[b]), (f.s.lastSelect = f.s.index === a && 1 === this.s.selectionList.length);
            else if (0 < c.length)
      ...
      b.prototype._paneDeclare = function (a, d, b) {
        ...
        for (var e = a.columns().eq(0).toArray().length, f = this.c.panes.length, g = 0; g < f; g++)
          this.s.panes.push(new m(d, b, e + g, this.c.layout, this.dom.panes, this.c.panes[g]));
        ...
        a.columns(0 < this.c.columns.length ? this.c.columns : void 0)
          ...
          .each(function (a) {
            if (void 0 !== d.s.panes[a] && void 0 !== d.s.panes[a].s.dtPane && void 0 !== d.s.panes[a].s.colOpts.preSelect)
              for (var c = d.s.panes[a].s.dtPane.rows().data().toArray().length, b = 0; b < c; b++)
                -1 !== d.s.panes[a].s.colOpts.preSelect.indexOf(d.s.panes[a].s.dtPane.cell(b, 0).data()) &&
  ...
  (function (b) {
    ...
    b(d).on("preInit.dt.dtsp", function (a, b, d) {
      "dt" === a.namespace && (b.oInit.searchPanes || c.defaults.searchPanes) && (b._searchPanes || f(b, !0));
    });
