...
- **Bucket routing:** `com.arcadedb.database.bucketselectionstrategy.BucketSelectionStrategy` (interface: `setType`, `getBucketIdByRecord(Document, boolean async)`, `getBucketIdByKeys(List<String> propertyNames, Object[] keyValues, boolean async)`, `getName`, `copy`), with `RoundRobinBucketSelectionStrategy`, `ThreadBucketSelectionStrategy`, `PartitionedBucketSelectionStrategy` implementations. The strategy is per type. `propertyNames` says which properties the key values belong to, so a strategy whose placement depends on the key can check they are its own before claiming a bucket, and must return -1 when it cannot (issue #5589); a strategy whose placement does not depend on the key ignores it.
...
- `getBucketIdByKeys(propertyNames, keyValues, async)` mirrors the record variant (tenant from context only; keys carry no tenant). It therefore **ignores `propertyNames`**: unlike `PartitionedBucketSelectionStrategy`, placement here does not depend on the key, so the tenant bucket is the right answer for a lookup on *any* index of the type, not just one. The parameter exists for the key-dependent strategies (issue #5589).
