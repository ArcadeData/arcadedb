...
A `MERGE … SET` analyzes to `{CREATE, UPDATE}`, so both `allowInsert` and `allowUpdate` must be enabled; `checkPermission` already throws a `SecurityException` naming whichever flag is missing. This is exactly the composition the pre-existing `MCPPermissionsTest.upsertRequiresBothCreateAndUpdate` stub asserts. When `setProperties`/`relProperties` are absent the statement has no `SET`, may analyze to `{CREATE}` alone, and then requires only `allowInsert` - a precise, more-permissive-than-strict outcome that is a deliberate consequence of reusing the analyzer rather than hard-coding "both always."
