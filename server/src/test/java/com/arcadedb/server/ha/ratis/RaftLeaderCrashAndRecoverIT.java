...
class RaftLeaderCrashAndRecoverIT extends BaseGraphServerTest {
  ...
  void oldLeaderRejoinsAsFollowerAfterRestart() {
    ...
    // Wait for the restarted peer to catch up
    ...
    final RaftHAServer oldLeaderHA = getServer(leaderIndex).getHA();
  ...
  private int waitForNewLeader(final int excludeIndex) {
    ...
    while (System.currentTimeMillis() < deadline) {
      for (int i = 0; i < getServerCount(); i++) {
        ...
        if (server != null && server.isStarted() && server.getHA() != null && server.getHA().isLeader())
          return i;
      }
      try {
        Thread.sleep(500);
