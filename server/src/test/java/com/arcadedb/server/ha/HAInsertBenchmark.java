...
public class HAInsertBenchmark {
  ...
  // ---------------------------------------------------------------------------
  //  Scenario: Cluster, embedded writes on leader
  // ---------------------------------------------------------------------------
  ...
  private String runClusterEmbeddedOnLeader(final int serverCount) throws Exception {
    ...
    try {
      ...
      insertEmbedded(db, WARMUP_COUNT, 0);
      final long[] latencies = insertEmbedded(db, MEASURE_COUNT, WARMUP_COUNT);
      return formatResult(serverCount + " servers (HA) - embedded on leader", latencies, 1);
    } finally {
      stopCluster(servers);
    }
  }
  ...
  // ---------------------------------------------------------------------------
  //  Scenario: Cluster, RemoteDatabase writes to a follower (HTTP proxy)
  // ---------------------------------------------------------------------------
  ...
  private String runClusterViaFollower(final int serverCount) throws Exception {
    ...
    try {
      ...
      try (final RemoteDatabase db = new RemoteDatabase("127.0.0.1", followerPort, DB_NAME, "root", ROOT_PASSWORD)) {
        ...
        return formatResult(serverCount + " servers (HA) - remote via follower proxy", latencies, 1);
      }
    } finally {
      stopCluster(servers);
    }
  ...
  // ---------------------------------------------------------------------------
  //  Scenario: Cluster, concurrent writes from all servers via RemoteDatabase
  // ---------------------------------------------------------------------------
  ...
  private ContextConfiguration serverConfig(final int index, final boolean haEnabled, final int serverCount) {
    ...
    config.setValue(GlobalConfiguration.SERVER_HTTP_INCOMING_PORT, String.valueOf(2480 + index));
    ...
    if (haEnabled) {
      ...
      for (int i = 0; i < serverCount; i++) {
        ...
        serverList.append("localhost:").append(2424 + i);
      }
