 ...
 */
@Tag("IntegrationTest")
class RaftLeaderDown2NodesIT extends BaseGraphServerTest {
  ...
  void remainingNodeCannotElectLeaderAfterLeaderDown() {
    ...
    final RaftHAServer replicaHA = getServer(replicaIndex).getHA();
    if (replicaHA != null) {
      ...
      try {
        ...
        try {
          ...
          LogManager.instance().log(this, Level.WARNING,
              "TEST: Write succeeded on remaining node - may have cached leader state");
        } catch (final TimeoutException e) {
          writeFuture.cancel(true);
          LogManager.instance().log(this, Level.INFO,
              "TEST: Write correctly timed out - no leader available");
        } catch (final Exception e) {
          LogManager.instance().log(this, Level.INFO,
              "TEST: Write correctly failed: %s", e.getMessage());
        }
      } finally {
