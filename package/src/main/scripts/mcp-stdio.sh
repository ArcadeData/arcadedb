...
# MCP Stdio Transport for ArcadeDB
# Starts the ArcadeDB server with MCP over stdin/stdout (JSON-RPC 2.0).
# Compatible with mcp-proxy, Claude Desktop, Cursor, etc.
...
[ -f "$ARCADEDB_HOME"/bin/mcp-stdio.sh ] || ARCADEDB_HOME=`cd "$PRGDIR/.." ; pwd`
