...
class MCPTransportConformanceTest extends BaseGraphServerTest {
  ...
  @BeforeEach
  void enableMCP() {
    final MCPConfiguration config = MCPPlugin.of(getServer(0)).getConfiguration();
  ...
  void configuredOriginIsAccepted() throws Exception {
    MCPPlugin.of(getServer(0)).getConfiguration().setAllowedOrigins(List.of("http://studio.example.com"));
    try {
      final Response response = post(new JSONObject()
          .put("jsonrpc", "2.0")
          .put("id", 1)
      ...
      MCPPlugin.of(getServer(0)).getConfiguration().setAllowedOrigins(List.of());
    }
  }
  ...
  private String runStdio(final String input) throws Exception {
    ...
    try (final PrintStream printStream = new PrintStream(out, true, StandardCharsets.UTF_8)) {
      new MCPStdioServer(getServer(0), MCPPlugin.of(getServer(0)).getConfiguration(), user,
